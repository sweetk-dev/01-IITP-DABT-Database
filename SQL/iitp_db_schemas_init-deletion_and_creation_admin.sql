...
-- ## ver 0.0.1 last updated data : 2025.07.22
-- ## Only for PostgreSQL
...
-- # public.sys_admin_account definition

DROP TABLE IF EXISTS public.sys_admin_account;


CREATE TABLE public.sys_admin_account (
	...
	login_id VARCHAR(40) NOT NULL,  -- admin login id
	password VARCHAR(40) NOT NULL,	 -- admin login password (encryption)
	...
	CONSTRAINT pkey_sys_admin_account PRIMARY KEY (adm_id)
);

CREATE UNIQUE INDEX uidx_admin_account_login ON public.sys_admin_account USING btree (login_id);
CREATE INDEX idx_admin_account_name ON public.sys_admin_account USING btree (name);
CREATE INDEX idx_admin_account_role_status ON public.sys_admin_account USING btree (roles, status);
COMMENT ON TABLE public.sys_admin_account IS 'IITP Admin Account(user) 정보 테이블';

COMMENT ON COLUMN public.sys_admin_account.adm_id IS ' system id, 고유 식별자 (자동 증가)';
COMMENT ON COLUMN public.sys_admin_account.login_id IS 'admin login id';
COMMENT ON COLUMN public.sys_admin_account.password IS 'admin login password (encryption)';
COMMENT ON COLUMN public.sys_admin_account.name IS 'admin name';

COMMENT ON COLUMN public.sys_admin_account.roles IS 'admin role, "sys_admin_roles" comm code 참조';
COMMENT ON COLUMN public.sys_admin_account.status IS '"data_status" comm code 참조';
COMMENT ON COLUMN public.sys_admin_account.del_yn IS '삭제여부 (Y: 삭제)';

COMMENT ON COLUMN public.sys_admin_account.affiliation IS 'admin 소속';
COMMENT ON COLUMN public.sys_admin_account.description IS 'admin 설명';
COMMENT ON COLUMN public.sys_admin_account.note IS '비고';

COMMENT ON COLUMN public.sys_admin_account.latest_login_at IS '마지막 로그인 시각';

COMMENT ON COLUMN public.sys_admin_account.created_at IS '레코드 생성 시각';
COMMENT ON COLUMN public.sys_admin_account.updated_at IS '레코드 수정 시각';
COMMENT ON COLUMN public.sys_admin_account.deleted_at IS '삭제 일시 (논리 삭제 시 기록)';
COMMENT ON COLUMN public.sys_admin_account.created_by IS '데이터 생성자 (SYS-BACH, SYS-MANUAL, user id), "sys_work_type" comm code 참조';
COMMENT ON COLUMN public.sys_admin_account.updated_by IS '데이터 수정자';
COMMENT ON COLUMN public.sys_admin_account.deleted_by IS '데이터 삭제자';
...
-- # public.sys_faq_info definition

DROP TABLE IF EXISTS public.sys_faq_info;

CREATE TABLE public.sys_faq_info (
	...
	CONSTRAINT pkey_sys_faq_info PRIMARY KEY (faq_id)
);
CREATE INDEX idx_sys_faq_info_sort_use ON public.sys_faq_info USING btree (sort_order,use_yn);
CREATE INDEX idx_sys_faq_info_type_q ON public.sys_faq_info USING btree (faq_type, question);



COMMENT ON TABLE public.sys_faq_info IS '자주 묻는 질문(FAQ) 정보 테이블';

COMMENT ON COLUMN public.sys_faq_info.faq_id      IS 'FAQ 고유 ID';
COMMENT ON COLUMN public.sys_faq_info.faq_type    IS 'FAQ 유형, "faq_type" comm code 참조';
COMMENT ON COLUMN public.sys_faq_info.question    IS 'FAQ 질문';
COMMENT ON COLUMN public.sys_faq_info.answer      IS 'FAQ 답변';
COMMENT ON COLUMN public.sys_faq_info.sort_order  IS '정렬 순서';
COMMENT ON COLUMN public.sys_faq_info.hit_cnt     IS 'FAQ 조회수';
COMMENT ON COLUMN public.sys_faq_info.use_yn      IS '사용 여부 (Y/N))';


COMMENT ON COLUMN public.sys_faq_info.created_by  IS '등록자 ID 또는 이름';
COMMENT ON COLUMN public.sys_faq_info.updated_by  IS '수정자 ID 또는 이름';
COMMENT ON COLUMN public.sys_faq_info.created_at  IS '등록 일시';
COMMENT ON COLUMN public.sys_faq_info.updated_at  IS '수정 일시';
