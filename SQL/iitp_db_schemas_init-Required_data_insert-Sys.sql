...
INSERT INTO public.sys_common_code (grp_id, grp_nm, code_id, code_nm, parent_grp_id, parent_code_id, code_type, code_lvl, sort_order, use_yn, del_yn, code_des, memo, created_at, created_by) 
    			...
    			('ext_sys_code', '외부 연동 시스템 코드', 'GBIS', '경기버스정보(GBIS)', null, null, 'S', 1, 1, 'Y'::bpchar, 'N'::bpchar, '버스 노선/배차 메타데이터 수집 (tran_bus_route_info)', '', CURRENT_TIMESTAMP, 'SYS-MANUAL'),
    			...
    			('ext_sys_code', '외부 연동 시스템 코드', 'GBIS_LOWFLOOR', '경기버스정보 저상버스 노선현황', null, null, 'S', 1, 7, 'Y'::bpchar, 'N'::bpchar, '전일 기준 저상버스 운행 노선 수집 (tran_bus_route_info.low_bus_yn)', '', CURRENT_TIMESTAMP, 'SYS-MANUAL');
...
-- 최초 로그인시 비밀번호 변경하도록 되어 있음!! 초기 비밀번호는 별도 전달
TRUNCATE TABLE  public.sys_adm_account;
-- ⚠️ 비밀번호 해시는 플레이스홀더입니다. 초기 구축 시 계정마다 서로 다른 값으로 생성해 넣으십시오.
--    예) SELECT crypt('<설정할 비밀번호>', gen_salt('bf', 10));   -- pgcrypto 확장 필요
INSERT INTO public.sys_adm_account (login_id, "password", "name", roles, status, del_yn, affiliation, description, note, created_at, created_by) 
			VALUES
			('sweetkSuper01@sweetk.co.kr', '<BCRYPT_HASH>', 'sweetk-super-root-01', 'S-ADMIN', 'A'::bpchar, 'N'::bpchar, 'sweetk', 'sweetk super admin', '', CURRENT_TIMESTAMP, 'SYS-MANUAL'),
			('sweetkRoot01@sweetk.co.kr', '<BCRYPT_HASH>', 'sweetk-root-01', 'ADMIN', 'A'::bpchar, 'N'::bpchar, 'sweetk', 'sweetk admin', '', CURRENT_TIMESTAMP, 'SYS-MANUAL');
...
INSERT INTO public.sys_ext_api_info (if_name, ext_sys, ext_url, auth, data_format, latest_sync_time, memo, del_yn, created_at, updated_at, deleted_at, created_by, updated_by, deleted_by) 
			values
				('KOSIS 국가통계포털', 'KOSIS', 'https://kosis.kr/statisticsList/statisticsListIndex.do?vwcd=MT_ZTITLE&menuId=M_01_01', '<KOSIS_API_KEY_BASE64>', '[JOSN,XML]', null, '', 'N'::bpchar, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, null, 'SYS-MANUAL', null, null);
