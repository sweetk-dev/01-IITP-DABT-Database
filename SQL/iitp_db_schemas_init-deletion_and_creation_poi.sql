...
-- ## ver 0.0.7 last update data : 2025.10.31
-- ## Only for PostgreSQL
...
CREATE TABLE public.poi_tour_bf_facility (
    ...
    toilet_yn         CHAR(1),                                              -- 장애인 화장실 여부(Y/N)
    elevator_yn       CHAR(1),                                              -- 엘리베이터 여부(Y/N)
    parking_yn        CHAR(1),                                              -- 장애인 주차장 여부(Y/N)
    slope_yn          CHAR(1),                                              -- 경사로 여부(Y/N)
    subway_yn         CHAR(1),                                              -- 지하철 접근성 여부(Y/N)
    bus_stop_yn       CHAR(1),                                              -- 버스 정류장 접근성 여부(Y/N)
    wheelchair_rent_yn CHAR(1),                                             -- 휠체어 대여 여부(Y/N)
    tactile_map_yn    CHAR(1),                                              -- 촉지도식 안내판 설치 여부(Y/N)
    audio_guide_yn    CHAR(1),                                              -- 오디오 가이드 제공 여부(Y/N)
    nursing_room_yn   CHAR(1),                                              -- 아기 돌봄/수유실 여부(Y/N)
    accessible_room_yn CHAR(1),                                             -- 무장애 객실 여부(Y/N)
    stroller_rent_yn  CHAR(1),                                              -- 유아차 대여 여부(Y/N)
...
CREATE INDEX idx_poi_tour_bf_facility_location ON public.poi_tour_bf_facility USING btree (latitude, longitude);
...
COMMENT ON COLUMN public.poi_tour_bf_facility.toilet_yn IS '장애인 화장실 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.elevator_yn IS '엘리베이터 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.parking_yn IS '장애인 주차장 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.slope_yn IS '경사로 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.subway_yn IS '지하철 접근성 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.bus_stop_yn IS '버스 정류장 접근성 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.wheelchair_rent_yn IS '휠체어 대여 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.tactile_map_yn IS '촉지도식 안내판 설치 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.audio_guide_yn IS '오디오 가이드 제공 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.nursing_room_yn IS '아기 돌봄/수유실 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.accessible_room_yn IS '무장애 객실 여부(Y/N)';
COMMENT ON COLUMN public.poi_tour_bf_facility.stroller_rent_yn IS '유아차 대여 여부(Y/N)';
