...
CREATE TABLE public.open_api_user (
	user_id int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL, -- system id, 고유 식별자 (자동 증가)
...
CREATE TABLE public.open_api_auth_key  (
	...
	updated_by varchar(40) NULL, -- 데이터 수정자
	deleted_by varchar(40) NULL, -- 데이터 삭제자
	CONSTRAINT pkey_open_api_auth_key  PRIMARY KEY (id)
);
...
COMMENT ON COLUMN public.open_api_auth_key.id IS 'system id, 고유 식별자 (자동 증가)';
