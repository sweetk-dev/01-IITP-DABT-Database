...
-- ## ver 0.0.3 last update data : 2025.07.10
-- ## Only for PostgreSQL
...
CREATE TABLE public.emp_dis_training_usage (
                                               ...
                                               org_category varchar(100) NOT NULL, -- 훈련기관구분
...
CREATE INDEX idx_emp_dis_training_usage_org_category ON public.emp_dis_training_usage (org_category);
...
COMMENT ON COLUMN public.emp_dis_training_usage.org_category IS '훈련기관구분';
COMMENT ON COLUMN public.emp_dis_training_usage.user_count IS '양성과정 이용자수';
COMMENT ON COLUMN public.emp_dis_training_usage.employed_count IS '양성과정 취업자수';
...
CREATE INDEX idx_emp_dis_training_course_type_name ON public.emp_dis_training_course (course_type, course_name);
...
COMMENT ON COLUMN public.emp_dis_training_course.apply_start_date IS '수강신청 시작일 (YYYY-MM-DD). 값이 없을 경우 "별도모집안내"로 표시';
COMMENT ON COLUMN public.emp_dis_training_course.apply_end_date IS '수강신청 종료일 (YYYY-MM-DD). 값이 없을 경우 "별도모집안내"로 표시';
...
CREATE INDEX idx_emp_dis_entre_lecture_year_title ON public.emp_dis_entre_lecture (year, title);
