...
CREATE TABLE public.emp_dis_dev_support_org (
	...
	day_activity boolean NOT NULL, -- 주간활동 (true/false)
	afterschool boolean NOT NULL, -- 청소년방과후활동 (true/false)
	indiv_plan boolean NOT NULL, -- 개인별지원계획 (true/false)
	parent_edu boolean NOT NULL, -- 부모교육 (true/false)
	family_rest boolean NOT NULL, -- 가족휴식 (true/false)
	parent_counsel boolean NOT NULL, -- 부모상담 (true/false)
	rights_relief boolean NOT NULL, -- 권리구제 (true/false)
	public_guardian boolean NOT NULL, -- 공공후견 (true/false)
	child_family_sup boolean NOT NULL, -- 장애아가족양육지원 (true/false)
	emergency_care boolean NOT NULL, -- 긴급돌봄 (true/false)
	created_at timestamptz DEFAULT CURRENT_TIMESTAMP NOT NULL, -- 생성시각
	updated_at timestamptz DEFAULT CURRENT_TIMESTAMP, -- 수정시각
	created_by varchar(40), -- 데이터 생성자 (SYS-BACH, SYS-MANUAL, BY-USER, admin name), "sys_work_type" comm code 참조
	updated_by varchar(40)  -- 데이터 수정자
...
COMMENT ON COLUMN public.emp_dis_dev_support_org.day_activity IS '주간활동 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.afterschool IS '청소년방과후활동 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.indiv_plan IS '개인별지원계획 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.parent_edu IS '부모교육 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.family_rest IS '가족휴식 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.parent_counsel IS '부모상담 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.rights_relief IS '권리구제 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.public_guardian IS '공공후견 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.child_family_sup IS '장애아가족양육지원 (true/false)';
COMMENT ON COLUMN public.emp_dis_dev_support_org.emergency_care IS '긴급돌봄 (true/false)';
...
CREATE TABLE public.emp_dis_obligation_status (
  ...
  workplace_count int4 NOT NULL, -- 대상 사업체 수 (개수)
